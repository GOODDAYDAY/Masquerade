  ...
  if (events.length === 0) {
    return (
      <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: 22, color: "#4b5563" }}>暂无发言</span>
      </div>
    );
  }
          ...
          <div key={idx}>
            ...
            <div style={{ opacity: isCurrent ? 1 : 0.7 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                <span style={{ width: 18, height: 18, borderRadius: "50%", backgroundColor: color, flexShrink: 0 }} />
                <span style={{ fontSize: 22, fontWeight: 500, color: "#d1d5db" }}>
                  {getName(item.event.player_id)}
                </span>
                {isLastWords && <span style={{ fontSize: 20, color: "#6b7280" }}>遗言</span>}
              </div>
              <div style={{
                marginLeft: 30, borderRadius: "19px 19px 19px 4px",
                padding: "16px 22px", fontSize: 22, lineHeight: 1.5,
